...
public class GymServiceImpl extends GymServiceGrpc.GymServiceImplBase {
    ...
    public void reserveGymSession(GymReservationRequest request,
        ...
        if (occupancy >= capacity) {
            responseObserver.onNext(GymReservationResponse.newBuilder()
                .setSuccess(false).setMessage("No hay cupos para " + request.getSessionType()).build());
        ...
        GymReservationInfo reservation = GymReservationInfo.newBuilder()
            ...
            .setTimeSlot(request.getTimeSlot()).setSessionType(request.getSessionType()).setStatus("CONFIRMED").build();
        
        reservations.put(reservationId, reservation);
        studentReservations.computeIfAbsent(request.getStudentId(), k -> new ArrayList<>()).add(reservationId);
        
        responseObserver.onNext(GymReservationResponse.newBuilder()
            .setSuccess(true).setReservationId(reservationId).setMessage("Reserva confirmada")
            .setConfirmedSlot(request.getTimeSlot()).build());
        responseObserver.onCompleted();
    }
    ...
    public void cancelGymReservation(CancelGymRequest request,
        ...
        if (reservation == null || !reservation.getStudentId().equals(request.getStudentId())) {
            responseObserver.onNext(CancelGymResponse.newBuilder().setSuccess(false)
                .setMessage("Reserva no encontrada").build());
            responseObserver.onCompleted();
            return;
        }
        
        currentOccupancy.put(reservation.getSessionType(), currentOccupancy.get(reservation.getSessionType()) - 1);
        reservations.remove(request.getReservationId());
        studentReservations.get(request.getStudentId()).remove(request.getReservationId());
        
        responseObserver.onNext(CancelGymResponse.newBuilder().setSuccess(true)
            .setMessage("Reserva cancelada").build());
        responseObserver.onCompleted();
    }
    ...
    public void getStudentReservations(StudentGymRequest request,
        ...
        GymReservationList.Builder builder = GymReservationList.newBuilder();
        
        for (String resId : studentResIds) {
            GymReservationInfo res = reservations.get(resId);
    ...
    public void getAvailableSessions(EmptyGymRequest request,
        ...
        List<String> slots = Arrays.asList("Lunes 07:00", "Lunes 17:00", "Martes 07:00", "Martes 18:00", "Miércoles 08:00");
        
        responseObserver.onNext(AvailableSessionsResponse.newBuilder()
            .addAllAvailableSlots(slots).putAllSessionCapacity(sessionCapacity).build());
        responseObserver.onCompleted();
    }
}